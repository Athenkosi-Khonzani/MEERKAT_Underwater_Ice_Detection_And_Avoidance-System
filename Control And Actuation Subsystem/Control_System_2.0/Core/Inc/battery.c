...
/* -------------------------------------------------------------------------
 * OCV → SOC lookup table for a generic 18650 Li-ion cell
 *
 * Columns: { OCV in volts, SOC in percent }
 * Must be sorted descending by voltage.
 * Extend / adjust points to match your cell's discharge curve if needed.
 * ------------------------------------------------------------------------- */
...
HAL_StatusTypeDef Battery_Update(void)
{
     ...
     * When the load current is small, the terminal voltage is close to OCV,
     * so we can gently pull the coulomb-counter estimate toward the OCV
     * estimate. Under heavy load, terminal voltage sags and the OCV method
     * is inaccurate, so the blend factor drops toward zero.
     *
     * Blend weight: 5 % when current < 50 mA, scales linearly to 0 % at 500 mA.
     * Adjust to taste.
     * ---------------------------------------------------------------------- */
    ...
    {
        /* First sample with real load — seed the EMA directly */
        s_avg_current_A = current;
    }
    else
    {
...
void Battery_GetData(BatteryData_t *out)
{
    ...
    if (s_avg_current_A > 0.005f)   /* 5 mA minimum to give a meaningful estimate */
    {
